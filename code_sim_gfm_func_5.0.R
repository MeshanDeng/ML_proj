...
dgp_gen_gfm <- function(n, T, r_signal=1, r_noise= NULL, n_noise_affected=n, alpha_vec=NULL, rho_cs=0.2, sigma_min=0.6, sigma_max=1.4){
  F_signal <- matrix(rnorm(T*r_signal), nrow=T, ncol=r_signal)
  Lambda_signal <- matrix(rnorm(n*r_signal), nrow=n, ncol=r_signal); Lambda_signal <- Lambda_signal / sqrt(mean(Lambda_signal^2))
  C_signal <- F_signal %*% t(Lambda_signal)
  E_common_noise <- matrix(0, nrow=T, ncol=n)
  if(r_noise>0){
    if(is.null(alpha_vec) || length(alpha_vec)!=r_noise) stop("alpha_vec must have length r_noise")
    G_noise <- matrix(rnorm(T*r_noise), nrow=T, ncol=r_noise)
    Lambda_noise <- matrix(0, nrow=n, ncol=r_noise)
    for(j in 1:r_noise){
      lam <- rnorm(n)
      if(n_noise_affected < n){ idx <- sample.int(n, n_noise_affected); lam[-idx] <- 0 }
      target_norm <- n^(alpha_vec[j]/2)
      cur_norm <- sqrt(sum(lam^2)); if(cur_norm==0) lam[sample.int(n,1)] <- 1
      lam <- lam * (target_norm / sqrt(sum(lam^2)))
      Lambda_noise[,j] <- lam
    }
...
dgp_make_y_lead_from_F <- function(F, h, gamma=1, sigma_v=1){
  T <- length(F); v <- rnorm(T, sd=sigma_v); y <- rep(NA_real_, T)
...
run_once <- function(n, T, rep_id, r_signal, r_noise, n_noise_affected, cfg, dgp_fun=dgp_gen_afm, model_funs=models_registry()){
  ...
  data <- dgp_fun(n,T,r_signal=r_signal,r_noise=r_noise,n_noise_affected=n_noise_affected,alpha_vec=cfg$alpha_vec)
   X0 <- data$X
  F <- as.numeric(data$F)
  Lambda <- data$Lambda
...
simulate_grid <- function(n_vals, k_ratio, R, r_signal, r_noise, noise_prop=NULL, noise_power= NULL, cfg, dgp_fun=dgp_gen_afm, model_funs=models_registry()){
  ...
  results_list <- future.apply::future_lapply(1:nrow(params_grid), function(i){
    ...
    r_noise_run <- min(r_noise, max(0L, n_run - r_signal))
    cfg_i <- cfg
    cfg_i$alpha_vec <- if(r_noise_run>0) cfg$alpha_vec[seq_len(r_noise_run)] else numeric(0)
    n_noise_run <- min(n_run, round(n_run^noise_power))
    run_once(n=n_run,T=T_run,rep_id=rep_id_run,r_signal=r_signal,r_noise=r_noise_run,n_noise_affected=n_noise_run,cfg=cfg_i,dgp_fun=dgp_fun,model_funs=model_funs)
  }, future.seed=TRUE)
...
plot_model_results <- function(agg, prefix, hyper_key=NULL, hyper_label=NULL, has_sparsity=FALSE, sparsity_key=NULL, main_tag=NULL){
  ...
  if(!is.null(hyper_key)){
    ...
    if(all(c(hy_m, hy_l, hy_u) %in% names(agg))){
      x   <- as.numeric(agg$n)
      y_m <- as.numeric(agg[[hy_m]])
      y_l <- as.numeric(agg[[hy_l]])
      y_u <- as.numeric(agg[[hy_u]])
      
      ok <- is.finite(x) & is.finite(y_m) & is.finite(y_l) & is.finite(y_u)
      
      if(sum(ok) < 2){
        plot.new()
        title(paste0("Hyperparameter (CV): ", ttl, " (insufficient)"))
      } else {
        plot(x[ok], y_m[ok], log="x", pch=19,
             ylim=range(c(y_l[ok], y_u[ok]), na.rm=TRUE),
             xlab="n",
             ylab=if(is.null(hyper_label)) hyper_key else hyper_label,
             main=paste0("Hyperparameter (CV): ", ttl))
        segments(x[ok], y_l[ok], x[ok], y_u[ok])
        lines(x[ok], y_m[ok], lwd=2)
      }
    } else {
...
plot_pls_proxy <- function(agg_mild, agg_hard, main_left="PLS dimensionality selection", main_right="Forecast alignment with oracle (PLS)"){
  merge_key <- intersect(agg_mild$n, agg_hard$n)
  M <- agg_mild[match(merge_key, agg_mild$n), ]
  H <- agg_hard[match(merge_key, agg_hard$n), ]
  op <- par(mfrow=c(1,2), mar=c(4,4,3,1)); on.exit(par(op), add=TRUE)
  
  ylim_nc <- range(c(1, M$ncomp_pls_l, M$ncomp_pls_u, H$ncomp_pls_l, H$ncomp_pls_u), na.rm=TRUE)
  plot(M$n, M$ncomp_pls_m, log="x", type="b", pch=19, lwd=2, ylim=ylim_nc, xlab="n", ylab="n_comp_best_pls", main=main_left)
  segments(M$n, M$ncomp_pls_l, M$n, M$ncomp_pls_u)
  lines(H$n, H$ncomp_pls_m, type="b", pch=2, lwd=2, col=2)
  segments(H$n, H$ncomp_pls_l, H$n, H$ncomp_pls_u, col=2)
  abline(h=1, lty=2, lwd=1.5)
  legend("topright", legend=c("MILD","HARD"), col=c(1,2), pch=c(19,2), lty=1, bty="n")
  
  ylim_corr <- range(c(M$corr_pls_or_l, M$corr_pls_or_u, H$corr_pls_or_l, H$corr_pls_or_u), na.rm=TRUE)
  plot(M$n, M$corr_pls_or_m, log="x", type="b", pch=19, lwd=2, ylim=ylim_corr, xlab="n", ylab="corr(ŷ_PLS, ŷ_oracle)", main=main_right)
  segments(M$n, M$corr_pls_or_l, M$n, M$corr_pls_or_u)
  lines(H$n, H$corr_pls_or_m, type="b", pch=2, lwd=2, col=2)
  segments(H$n, H$corr_pls_or_l, H$n, H$corr_pls_or_u, col=2)
  abline(h=1, lty=2)
  legend("bottomright", legend=c("MILD","HARD"), col=c(1,2), pch=c(19,2), lty=1, bty="n")
}
